  ...
  if (req.method === 'DELETE') {
    try {
      ...
      const db = client.db('car_rental_db');
       ...
       const existingCar = await db.collection('cars').findOne({ _id: ObjectId(carId) });
       if (!existingCar) {
      ...
      const result = await db.collection('cars').deleteOne({ _id: ObjectId(carId) });
      ...
      res.status(500).json({ message: 'Something went wrong' });
       ...
       try {
           ...
           if (!make || !model || !year || !color || !registration_number || !price_per_day || availability_status === undefined) {
            ...
            const currentCar = await db.collection('cars').findOne({ _id: ObjectId(carId) });
             if (!currentCar) {
